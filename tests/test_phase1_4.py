"""
Test Script for Phase 1.4 - Bronze Data Import v3.1
测试SAS文件导入到Bronze层
"""
...
def test_sas_import_mock():
    ...
    db_path = "test_bronze_import_v31.duckdb"
    ...
    mock_dm_data = pd.DataFrame(
        {
            ...
            "brthdat": [18000.0, 17800.0, 19000.0],
...
def test_with_real_sas():
    ...
    sas_dir = Path(__file__).parent.parent / "examples" / "D8318N00001" / "data"
    ...
    db_path = "test_real_sas_v31.duckdb"
    sql_script = str(Path(__file__).parent.parent / "sql" / "init_meta.sql")
    als_file = str(
        Path(__file__).parent.parent
        / "examples"
    ...
    try:
        result = load_sas_to_bronze(
            sas_path=str(first_sas), form_oid=form_oid, db=db, validate=True
        )
...
def test_batch_load():
    ...
    db_path = "test_batch_load_v31.duckdb"
    sql_script = str(Path(__file__).parent.parent / "sql" / "init_meta.sql")
    als_file = str(
        Path(__file__).parent.parent
        / "examples"
    ...
    try:
        summary = load_study_data(data_dir=str(sas_dir), db=db, validate=True)
        ...
        if summary["successful"] > 0:
            ...
            for result in summary["results"]:
                print(
                    f"     - {result['table_name']}: {result['inserted_records']} records"
                )
...
def main():
    """运行所有测试"""
    print("=" * 60)
    print("Phase 1.4 Testing Suite v3.1")
    print("Bronze Data Import")
    print("=" * 60)
    ...
    for db_file in [
        "test_bronze_import_v31.duckdb",
        "test_real_sas_v31.duckdb",
        "test_batch_load_v31.duckdb",
    ]:
        if Path(db_file).exists():
            print(f"   - {db_file}")

    print("\n[TIP] To inspect databases:")
    print("   duckdb test_bronze_import_v31.duckdb")

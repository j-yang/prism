"""
PRISM-DB Silver Layer SQL Generator

Generate SQL scripts for Bronze → Silver derivation.
Uses hardcoded templates for simple patterns, GLM-5 for complex ones.
"""
...
class SilverGenerator:
    """Generate Silver layer SQL from metadata."""
...
WHERE paramcd IS NOT NULL;""",
    ...
    def _load_derivations(self) -> Dict[str, List[Dict]]:
        """Load derivations from meta.derivations, grouped by schema."""
        sql = """
    ...
    def _generate_single(self, deriv: Dict, bronze_schema: Dict) -> Optional[str]:
        """Generate SQL for a single derivation."""
    ...
    def _try_template(self, deriv: Dict) -> Optional[str]:
        """Try to match a hardcoded template."""
        transformation = deriv.get("transformation", "")
    ...
    def _render_template(self, template: str, deriv: Dict) -> str:
        """Render a template with derivation data."""
        import pandas as pd
    ...
    def _call_llm(self, deriv: Dict, bronze_schema: Dict) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"LLM call failed for {deriv['deriv_id']}: {e}")
            return None
...
-- Rule: {deriv.get("transformation", "")[:100]}..."""
    ...
    def _placeholder_sql(self, deriv: Dict) -> str:
        """Generate placeholder SQL for skipped derivations."""

"""
PRISM-DB Gold Layer Analysis Engine

Generate Python analysis scripts for Silver → Gold aggregation.
Uses templates for simple outputs, LLM for complex ones.
"""
...
class GoldEngine:
    """
    Generate Python analysis scripts for Gold layer.

    Flow:
    1. Read meta.outputs for output definitions
    2. Generate Python script using templates or LLM
    3. User executes script to compute statistics
    4. Results written to gold.{schema} tables
    """
    ...
    def generate_all(self, output_dir: str = "generated/analysis/") -> Dict[str, Any]:
        """
        Generate Python analysis scripts for all outputs.
    ...
    def _load_outputs(self) -> List[Dict]:
        """Load output definitions from meta.outputs."""
        import pandas as pd
    ...
    def _generate_single(self, output: Dict) -> Optional[str]:
        """Generate Python script for a single output."""
    ...
    def _try_template(self, output: Dict) -> Optional[str]:
        """Try to match a template for the output."""
        output_id = output["output_id"]
        title = output.get("title", "")
        schema = output.get("schema", "baseline")
    ...
    def _render_template(self, template_name: str, output: Dict) -> str:
        """Render a template for the output."""
        output_id = output["output_id"]
        schema = output.get("schema", "baseline")
        title = output.get("title", "")
        population = output.get("population", "SAFFL")
    ...
    def _demographics_template(
        ...
        """Generate demographics table script."""
        var_defs = "\n".join(
            [
                f"    {{'name': '{v.lower()}', 'type': 'continuous'}},"
                for v in variables
                if v.lower() in ["age", "weight", "height", "bmi"]
            ]
        )
        var_defs += "\n".join(
            [
                f"    {{'name': '{v.lower()}', 'type': 'categorical'}},"
                for v in variables
                if v.lower() in ["sex", "race", "ethnic"]
            ]
        )

        select_cols = ["usubjid", "trta", population.lower()]
        for v in variables:
            if v.lower() not in select_cols:
                select_cols.append(v.lower())
...
import pandas as pd
import numpy as np
from prismdb import Database
from prismdb.gold.stats import desc_stats_continuous, desc_stats_categorical, format_stat
...
{var_defs}
    ...
    # 3. Compute statistics by treatment
    results = []
    ...
    for trta in df['trta'].unique():
        ...
        for var in ['age', 'weight', 'height']:
            if var in df.columns:
                stats = desc_stats_continuous(subset[var].dropna().tolist())
...
'''
    ...
    def _ae_summary_template(
        ...
        """Generate AE summary table script."""
        return f'''"""
{title}
Output ID: {output_id}
Schema: {schema}
...
def main():
    ...
    # 2. Compute summary by treatment
    results = []
...
'''
    ...
    def _call_llm(self, output: Dict) -> Optional[str]:
        """Call LLM to generate analysis script."""
        if not self.api_key:
            logger.warning(
        ...
        try:
            ...
            logger.error(f"LLM call failed for {output['output_id']}: {e}")
            return None
...
'''
    ...
    def _placeholder_script(self, output: Dict) -> str:
        """Generate placeholder script for skipped outputs."""
        output_id = output["output_id"]
        schema = output.get("schema", "baseline")
        title = output.get("title", "")

"""
PRISM-DB Gold Statistics Functions

Statistical functions for computing Gold layer aggregations.
Uses only numpy/pandas for MVP.
"""
...
def desc_stats_continuous(values: List[float]) -> Dict[str, Any]:
    ...
    Returns:
        Dict with n, mean, sd, median, min, max
    """
...
def desc_stats_categorical(values: List[str]) -> List[Dict[str, Any]]:
    """
    Calculate frequency statistics for categorical variable.

    Args:
        values: List of category values

    Returns:
        List of dicts with category, n, pct
    """
...
def compute_table_stats(
    ...
    """
    Compute statistics for a table output.

    Args:
        df: DataFrame with silver layer data
        variables: List of variable definitions [{'name': 'age', 'type': 'continuous'}, ...]
        group_col: Column to group by
        filter_condition: Optional filter condition

    Returns:
        List of statistic records for gold table
    """
    if filter_condition:
...
def compute_total_stats(
    ...
    """
    Compute total column statistics (all subjects combined).

    Args:
        df: DataFrame with silver layer data
        variables: List of variable definitions
        group_col: Column to identify groups (for reference)
        filter_condition: Optional filter condition
    ...
    for var in variables:
        ...
        if var_type == "continuous":
            stats = desc_stats_continuous(df[var_name].tolist())
            ...
            stats = desc_stats_categorical(df[var_name].tolist())
